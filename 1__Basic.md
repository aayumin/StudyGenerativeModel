...
2. Sampling: 잠재 공간에서 무작위로 샘플을 추출
...
이 싸움을 반복하면서 G는 점점 더 진짜 같은 돈을 만들어냅니다.
...
| 모델 | 구조 | 특징 | 단점 |
|------|------|------|------|
| VAE | 인코더-디코더 | 잠재공간 학습, 다양한 샘플 생성 | 이미지 품질이 낮은 편입니다. |
| GAN | 생성자 vs 판별자 | 사실적인 이미지 생성 | 학습이 불안정하고, 다양성 부족 문제가 있습니다. |
| Diffusion | 순방향 노이즈 + 역방향 제거 | 매우 고품질 이미지 생성 | 속도가 느리고, 자원 소모가 큽니다. |
